...
def add_subscription(obj, user):
    ...
    """
    if user.is_staff:
        subscription, is_created = Subscriber.objects.get_or_create(subscriber=user, follower=obj)
        return subscription
        ...
        subscription, is_created = Subscriber.objects.get_or_create(subscriber=user, follow_requests=obj)
        return subscription
...
def confirm_subscription_everybody(obj, user):
    if user.is_authenticated and Page.objects.filter(id=obj.id, owner=user):
        ...
        for everybody in follow_requests:
            subscription, is_created = Subscriber.objects.get_or_create(subscriber=everybody, follower=obj)    
            Subscriber.objects.filter(subscriber=everybody, follow_requests=obj).delete()
            return subscription

# def confirm_subscription_one_user(obj, user):
#     if user.is_authenticated and Page.objects.filter(id=obj.id, owner=user):
#         one_user = User.objects.filter(subscribers__follow_requests=obj).get() # pk=???????????????????????????
#         subscription, is_created = Subscriber.objects.get_or_create(subscriber=one_user, follower=obj)    
#         Subscriber.objects.filter(subscriber=one_user, follow_requests=obj).delete()
#         return subscription
...
def is_follow_requests(obj, user):
    ...
    if not user.is_authenticated:
        return False
    subscriber = Subscriber.objects.filter(subscriber=user, follow_requests=obj)
    return subscriber.exists()
